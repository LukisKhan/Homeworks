...
function reverseString(str) {
    ...
    return str[lastCharIdx] += reverseString(str.slice(0, lastCharIdx))
}
...
function pow(base, exponent) {
    ...
    return exponent > 0 ? (base * pow(base, exponent - 1)) : (1/base * pow(base, exponent + 1))
}
...
function flatten(data, result = []) {
    ...
    data.forEach(ele => {
        if(Array.isArray(ele)) {
            return result.concat(flatten(ele, result))     
        } else {
            result.push(ele)
        }
    })
...
function fileFinder(directories, targetFile) {
    ...
    let arr = Object.keys(directories)
    for(let i = 0; i < arr.length; i++) {
        let subdir = directories[arr[i]]
        // console.log(Object.keys(subdir))
        // console.log( subdir)
        // console.log( typeof subdir)
        // console.log(Object.keys(subdir).includes(targetFile))
        if ( typeof subdir === 'object' && subdir !== null ) {
            // let bool = Object.keys(subdir).includes(targetFile)
            // return bool ? true : fileFinder( subdir, targetFile )
            // console.log(Object.keys(subdir))
            // console.log(targetFile)
            // console.log(Object.keys(subdir).includes(targetFile))
            // if (Object.keys(subdir).includes(targetFile)){
            //     return true
            // }
            if(fileFinder(subdir, targetFile) === true) return true;
            // boolArray.push(fileFinder(subdir, targetFile))
    ...
    // .forEach(subDir => {
    //     console.log(subDir)
    //     if (directories[subDir] === null || subDir === 0 ) return;
    //     else {
    //         return Object.keys(subDir).includes(targetFile) ? true : fileFinder(subDir, targetFile)
    //     }
    //     // subDir ? fileFinder(subDir, targetFile) : ""
    // })
    // return false
}
...
function pathFinder(directories, targetFile, filePath = '') {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        filePath += arr[i]
        if (typeof subdir === 'object' && subdir !== null) {
            const path = (pathFinder(subdir, targetFile, filePath)) 
            if ( path !== null) return arr[i] + path
        } 
    }
    return null;
}
