...
def just_like_orange
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
    --SELF JOIN Orange's Breed
    -- TableA is not_orange
    -- TableB is is_orange
    -- JOINing on is_orange with breed
    -- Executing a INNER JOIN on not_orange(left) and is_orange(right)
    -- Ordering, return results
    SELECT not_orange.name, not_orange.breed
    FROM cats AS not_orange
    JOIN cats AS is_orange ON is_orange.breed = not_orange.breed
    WHERE not_orange.name != 'Orange' AND is_orange.name = 'Orange'
    ORDER BY not_orange.name ASC;
  SQL
end
  # Table A only Orange
  # Table B every cat except Orange
...
def toys_that_jet_owns
  ...
  # SOLUTION FOR toys_that_jet_owns
  # SELECT DISTINCT
  #   notjetcats.name, toys.name 
  # FROM 
  #   toys 
